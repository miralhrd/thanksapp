...
async function loadRosterFull(ver) {
  var ro = await GU.ensureRosterFull(ver);
...
async function populateFromLogin(uid, password, r) {
  ...
  await loadRosterFull(r.rosterVersion);
  var d = GU.dataCache.load(uid);
  if (d) { d.lastReadTs = Math.max(d.lastReadTs || 0, r.lastReadTs || 0); GUD.data = d; }
  else GUD.data = null;
}
async function legacyLoginAndLoad(uid, password) {
  if (!uid) return { ok: false, error: "명단에서 이름을 다시 선택해주세요." };
  var r = await GU.api("loginAndLoad", { uid: uid, password: password });
...
async function apiCall(p) {
  switch (p.action) {
    ...
    case "setPassword":
      return GU.api("setPassword", { uid: uidOfFull(p.name), password: p.password });
    case "login": {
